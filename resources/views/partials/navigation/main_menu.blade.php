...
Wanneer dit waar is komt ereen active class. Ook URL:: is een goede helper ivm het vinden van de correcte folder--}}
	...
	<li class="{{ active_class(if_route(['home']) || if_uri(['/'])) }}">
	...
	<li class=" dropdown">
      ...
      <ul class="dropdown-menu">
		<li><a href="../activiteiten">Activiteiten van HMV Actis</a></li>
      	<li><a href="../albums">Foto's van activiteiten</a></li>
      </ul>
     </li>
     @else
	...
	<li class="{{ active_class(if_route(['vacatures']) || if_uri(['vacatures'])) }}">
		<a href="{{ URL::to('vacatures') }}">VACATURES</a>
	</li>

	<li class="{{ active_class(if_route(['contact']) || if_uri(['contact'])) }}">
...
@if (Auth::check())
	...
	@if($profiel->isEmpty())
		...
		<li class="{{ active_class(if_route(['registreren']) || if_uri(['registreren'])) }}">
			<a href="{{ URL::to('registreren') }}">WORD LID</a>
		</li>
	@else

		<li class="{{ active_class(if_route(['profiel']) || if_uri(['profiel'])) }}">
			<a href="{{ URL::to('profiel') }}">PROFIEL</a>
		</li>

	@endif

		<li class="{{ active_class(if_route(['logout']) || if_uri(['logout'])) }}">
			<a href="{{ URL::to('logout') }}">UITLOGGEN</a>
		</li>
@else

	<li class="{{ active_class(if_route(['login']) || if_uri(['login'])) }}">
		<a href="{{ URL::to('login') }}">INLOGGEN</a>
	</li>

	<li class="{{ active_class(if_route(['registreren']) || if_uri(['registreren'])) }}">
		<a href="{{ URL::to('registreren') }}">LID WORDEN</a>
	</li>
